...
export default async function QuotePage({
  ...
  const { data: quoteRow } = await supabase
    .from("quotes")
    .select(
      "id, business_id, client_id, quote_number, status, issued_at, sent_at, valid_until, notes, subtotal, tax_amount, total, public_token, created_at, updated_at",
    )
    .eq("id", id)
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <p className="text-center text-sm text-muted">
        שליחה בוואטסאפ ומעקב אחרי צפיות יתווספו בשלבים הבאים.
      </p>
    </div>
  );
}
